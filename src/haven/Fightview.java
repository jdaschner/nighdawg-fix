...
import haven.custom.AutoGiveButton;
import haven.custom.GameUIUtils;
...
public class Fightview extends Widget {
	...
	public class Relation {
		...
		public void use(Indir<Resource> act) {
			...
			try {
				...
				if (lastact.get() != null
						&& (Config.nonAttackDefences.keySet().stream().anyMatch(lastact.get().name::matches))) {
	...
	public class Relbox extends Widget {
		...
		public Relbox(Relation rel) {
			super(bg.sz());
			this.rel = rel;
			Widget avaf = adda(Frame.with(ava = new Avaview(avasz, rel.gobid, "avacam"), true), UI.scale(25), sz.y / 2,
					0.0, 0.5);
			ava.canactivate = true;
			add(give = new GiveButton(0, UI.scale(15, 15)), UI.scale(5, 4));
			adda(purs = new Button(UI.scale(70), "Pursue"), avaf.c.x + avaf.sz.x + UI.scale(5),
					avaf.c.y + (avaf.sz.y / 2), 0.0, 0.5);
			rel.give = give;
		}
	...
	public void tick(double dt) {
		...
		for (Relation rel : lsrel) {
			...
			if (inf != null)
				inf.tick(dt);
			try {
				if (OptWnd.autoPeaceAnimalsWhenCombatStartsCheckBox.a && !rel.autopeaced && curdisp != null
						&& curdisp.give != null && curdisp.give.state != 1) {
		...
		if (msg == "new") {
			...
			if(rel == current) {
				...
				if (rel.autogive != null && rel.autogive.state == 1 && ui != null && ui.gui != null) {
					...
					if (!GameUIUtils.aggro(ui.gui, rel.gobid)) {
						autogivegobid = -1;
					}
				}
			}
	...
	public void playCombatSoundEffect(Indir<Resource> lastact) {
		if (lastact != null) {
			try {
				...
				if (tt == null) { // ND: Took this code from Matias. I wonder what tooltip it's referring to.
									// Maybe some combat moves had some missing stuff in the past?
					ui.gui.syslog.append(
							"Combat: WARNING! tooltip is missing for " + res.name + ". Notify Jorb/Loftar about this.",
							new Color(234, 105, 105));
					return;
				}

				if (OptWnd.cleaveSoundEnabledCheckbox.a && res.basename().endsWith("cleave")) {
					try {
						File file = new File(haven.MainFrame.gameDir + "AlarmSounds/"
								+ OptWnd.cleaveSoundFilename.buf.line() + ".wav");
						if (!file.exists()) {
							return;
						}
						AudioInputStream in = AudioSystem.getAudioInputStream(file);
						AudioFormat tgtFormat = new AudioFormat(AudioFormat.Encoding.PCM_SIGNED, 44100, 16, 2, 4, 44100,
								false);
						AudioInputStream pcmStream = AudioSystem.getAudioInputStream(tgtFormat, in);
						Audio.CS klippi = new Audio.PCMClip(pcmStream, 2, 2);
						((Audio.Mixer) Audio.player.stream)
								.add(new Audio.VolAdjust(klippi, OptWnd.cleaveSoundVolumeSlider.val / 50.0));
					} catch (Exception e) {
						e.printStackTrace();
					}
					...
					try {
						File file = new File(
								haven.MainFrame.gameDir + "AlarmSounds/" + OptWnd.opkSoundFilename.buf.line() + ".wav");
						if (!file.exists()) {
							return;
						}
						AudioInputStream in = AudioSystem.getAudioInputStream(file);
						...
						Audio.CS klippi = new Audio.PCMClip(pcmStream, 2, 2);
						((Audio.Mixer) Audio.player.stream)
								.add(new Audio.VolAdjust(klippi, OptWnd.opkSoundVolumeSlider.val / 50.0));
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
