...
public class Widget {
	...
	public static class HandlerMaker extends Resource.PublishedCode.Instancer.Chain<MessageHandler> {
...

		{
			...
			add(new StaticCall<>(MessageHandler.class, "uimsg", Void.TYPE,
					new Class<?>[] { Widget.class, Object[].class },
	...
	public static class MouseDownEvent extends MouseButtonEvent {
		...
		public MouseDownEvent derive(Coord c) {
			return (new MouseDownEvent(this, c));
		}
	...
	public static class MouseUpEvent extends MouseButtonEvent {
		...
		public MouseUpEvent derive(Coord c) {
			return (new MouseUpEvent(this, c));
		}
	...
	public static class MouseMoveEvent extends MouseEvent {
		...
		public MouseMoveEvent derive(Coord c) {
			return (new MouseMoveEvent(this, c));
		}
	...
	public static class MouseWheelEvent extends MouseActionEvent {
		...
		public MouseWheelEvent derive(Coord c) {
			return (new MouseWheelEvent(this, c));
		}
	...
	public static class MouseHoverEvent extends MouseEvent {
		...
		public MouseHoverEvent derive(Coord c) {
			return (new MouseHoverEvent(this, c));
		}
	...
	public static class TooltipQuery extends PointerEvent {
		...
		public TooltipQuery derive(Coord c) {
			return (new TooltipQuery(this, c));
		}
	...
	public Position pos(String nm) {
		...
		if (ret == null)
			throw (new IllegalArgumentException(
					String.format("Illegal position anchor \"%s\" from widget %s", nm, this)));
		return (ret);
	}
				...
				if (base != null) {
					if (rich) {
						...
						if ((key != null) && (key != KeyMatch.nil))
							tip = String.format("%s\n\nKeyboard shortcut: $col[255,200,0]{%s}", tip,
									RichText.Parser.quote(key.longname())); // ND: all 3 of these affect the keybind
																			// text color
						w = UI.scale(300);
					} else {
						tip = RichText.Parser.quote(base);
						if ((key != null) && (key != KeyMatch.nil))
							tip = String.format("%s ($col[255,200,0]{%s})", tip, RichText.Parser.quote(key.longname())); // ND:
																															// all
																															// 3
																															// of
																															// these
					...
					if ((key == null) || (key == KeyMatch.nil))
						...
						tip = String.format("Keyboard shortcut: $col[255,200,0]{%s}",
								RichText.Parser.quote(key.longname())); // ND: all 3 of these affect the keybind text
																		// color
				}
